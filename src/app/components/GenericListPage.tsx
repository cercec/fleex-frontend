...
const GenericListPage: React.FC<Props> = ({dataType, showFilter, customData}) => {
    /* Vars */
    const [data, setData] = useState<User[] | Product[]>(customData || []);
    ...
    const isOneAdminRemaining = dataType === DataType.Users && (data as User[]).filter((user: User) => user.userRole === 'admin').length === 1;
            ...
            {(dataType === DataType.Products ? filteredProducts : data).length > 0 ? (
                <div className="overflow-x-auto">
                    <table className="table">
                        ...
                        <tbody>
                        {(filteredProducts ?? data).map((item: User | Product) => {
                                            ...
                                            return (
                                                <td key={index}>
                                                    ...
                                                    <button className="btn btn-red"
                                                            disabled={disableDeleteForUsers && isOneAdminRemaining}
                        ...
                        <div className="bg-white rounded-lg shadow-xl">
                            <div className="p-4">
                                {dataType === DataType.Users ? (
                                    <ProfileForm mode="add" handleSubmit={handleAdd as (user: User) => void}
                                                 confirmText="Ajouter" handleClose={() => setShowModal(false)}
                                                 title="Ajouter un profil"/>
                                ) : (
                                    <ProductForm handleSubmit={handleAdd as (product: Product) => void}
                                                 confirmText="Ajouter" handleClose={() => setShowModal(false)}
